...
def statistics(l,N,T,J,kb,eq,block_size,beta,T_init,corr_end):
    ...
    tau_m=1/chi_m[0]*np.sum(chi_m[j] for j in range(0,int(corr_end*N)))
    print('tau_m:',tau_m)
...
N=np.array([1000,1000,1000,1000,1000,1000,1000,10000,10000,3000,3000,2000,1000,1000,1000,1000,1000,1000,1000]) #number of sweeps
eq=np.array([200,200,200,200,200,200,200,5000,5000,1000,1000,400,200,200,200,200,200,200,200]) # point where equilibrium is assumed to have been reached
corr_ends=np.array([0.5,0.5,0.5,0.5,0.5,0.5,0.8,0.9,0.9,0.9,0.9,0.9,0.5,0.5,0.5,0.5,0.5,0.5,0.5])
...
for i in range(T.size):
    U_means[i],U_stds[i],Mag_means[i],Mag_stds[i],c_means[i],c_stds[i],tau_ms[i]=statistics(l,N[i],T[i],J,kb,eq[i],block_size,beta[i],T_init,corr_ends[i])
t2=ti.time()
