...
def statistics(l,N,T,J,kb,eq,block_size,beta,T_init):
    ...
    print('T:',T)
    return U_mean,U_std,Mag_mean,Mag_std,c_mean,c_std
...
for i in range(T.size):
    U_means[i],U_stds[i],Mag_means[i],Mag_stds[i],c_means[i],c_stds[i]=statistics(l,N,T[i],J,kb,eq,block_size,beta[i],T_init)
t2=ti.time()
